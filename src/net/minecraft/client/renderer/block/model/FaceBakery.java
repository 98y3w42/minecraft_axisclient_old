...
public class FaceBakery
{
    ...
    private void func_178408_a(int[] p_178408_1_, EnumFacing p_178408_2_)
    {
        ...
        for (int k = 0; k < 4; ++k)
        {
            ...
            if (f1 < afloat[Constants.WEST_INDEX])
            {
                afloat[Constants.WEST_INDEX] = f1;
            }

            if (f2 < afloat[Constants.DOWN_INDEX])
            {
                afloat[Constants.DOWN_INDEX] = f2;
            }

            if (f < afloat[Constants.NORTH_INDEX])
            {
                afloat[Constants.NORTH_INDEX] = f;
            }

            if (f1 > afloat[Constants.EAST_INDEX])
            {
                afloat[Constants.EAST_INDEX] = f1;
            }

            if (f2 > afloat[Constants.UP_INDEX])
            {
                afloat[Constants.UP_INDEX] = f2;
            }

            if (f > afloat[Constants.SOUTH_INDEX])
            {
                afloat[Constants.SOUTH_INDEX] = f;
            }
        }
