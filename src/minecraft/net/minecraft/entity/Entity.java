...
public abstract class Entity implements ICommandSender {
	...
	protected void setSize(float width, float height) {
		if (width != this.width || height != this.height) {
			...
			this.width = width;
			this.height = height;
			this.setEntityBoundingBox(new AxisAlignedBB(this.getEntityBoundingBox().minX, this.getEntityBoundingBox().minY, this.getEntityBoundingBox().minZ, this.getEntityBoundingBox().minX + (double) this.width,
	...
	public void moveEntity(double x, double y, double z) {
		if (this.noClip) {
			...
			walkingevent.call();
			boolean flag = this.onGround && this instanceof EntityPlayer && (walkingevent.getSafeWalk() || this.isSneaking());
			if (flag) {
				double d6;
			...
			if (this.stepHeight > 0.0F && flag1 && (d3 != x || d5 != z)) {
				StepEvent event = new StepEvent(this.stepHeight, flag1, Event.State.PRE);
				event.call();
				...
				if (d11 * d11 + d8 * d8 >= x * x + z * z) {
					...
					if (event.getEdit()) {
						if ((this.stepHeight > 0.0F) && flag1 && (d3 != x || d5 != z)) {
							...
							y = event.stepHeight;
							List var301 = this.worldObj.getCollidingBoundingBoxes(this, getEntityBoundingBox().addCoord(d3, y, d5));
							AxisAlignedBB var311 = getEntityBoundingBox();
							AxisAlignedBB var321 = var311.addCoord(d3, 0.0D, d5);
							double var331 = y;
							...
							if (var731 > var481) {
								x = var671;
								z = var681;
								setEntityBoundingBox(var311);
							} else {
								x = var711;
								z = var721;
								setEntityBoundingBox(var691);
							}
							y = -this.stepHeight;
							...
							if (var561 * var561 + var271 * var271 >= x * x + z * z) {
								...
								y = var251;
								z = var271;
								setEntityBoundingBox(var291);
							} else {
								StepEvent eventConfirm = new StepEvent(Event.State.POST);
								eventConfirm.call();
							}
						}
					}
	...
	public int getBrightnessForRender(float partialTicks) {
		...
		return 100;// this.worldObj.isBlockLoaded(blockpos) ?
	...
	public MovingObjectPosition rayTrace(double blockReachDistance, float partialTicks) {
		RayTraceEvent e = new RayTraceEvent(blockReachDistance, partialTicks);
		e.call();
		blockReachDistance = e.reach;
	...
	public boolean isEntityInsideOpaqueBlock() {
		if (this.noClip) {
			 ...
			 * BlockPos.MutableBlockPos(Integer.MIN_VALUE, Integer.MIN_VALUE,
			 * Integer.MIN_VALUE);
			 *
			 * for (int i = 0; i < 8; ++i) { int j =
			 * MathHelper.floor_double(this.posY + (double)(((float)((i >> 0) %
			 * 2) - 0.5F) * 0.1F) + (double)this.getEyeHeight()); int k =
			 * MathHelper.floor_double(this.posX + (double)(((float)((i >> 1) %
			 * 2) - 0.5F) * this.width * 0.8F)); int l =
			 * MathHelper.floor_double(this.posZ + (double)(((float)((i >> 2) %
			 * 2) - 0.5F) * this.width * 0.8F));
			 *
			 ...
			 *
			 * if
			 * (this.worldObj.getBlockState(blockpos$mutableblockpos).getBlock()
			 * .isVisuallyOpaque()) { return true; } } }
			 */
