...
public class LayerCape implements LayerRenderer<AbstractClientPlayer> {
	...
	public void doRenderLayer(AbstractClientPlayer entitylivingbaseIn, float p_177141_2_, float p_177141_3_, float partialTicks, float p_177141_5_, float p_177141_6_, float p_177141_7_, float scale) {
		...
		if ((entitylivingbaseIn.hasPlayerInfo() || friend) && !entitylivingbaseIn.isInvisible() && (entitylivingbaseIn.isWearing(EnumPlayerModelParts.CAPE) || friend) && (entitylivingbaseIn.getLocationCape() != null || friend)) {
			...
			this.playerRenderer
					.bindTexture(Axis.getAxis().getFriendManager().isFriend(StringUtils.stripControlCodes(entitylivingbaseIn.getName())) ? new ResourceLocation("textures/axis/axiscape.png") : entitylivingbaseIn.getLocationCape());
			GlStateManager.pushMatrix();
			GlStateManager.translate(0.0F, 0.0F, 0.125F);
			double d0 = entitylivingbaseIn.prevChasingPosX + (entitylivingbaseIn.chasingPosX - entitylivingbaseIn.prevChasingPosX) * (double) partialTicks
					- (entitylivingbaseIn.prevPosX + (entitylivingbaseIn.posX - entitylivingbaseIn.prevPosX) * (double) partialTicks);
			double d1 = entitylivingbaseIn.prevChasingPosY + (entitylivingbaseIn.chasingPosY - entitylivingbaseIn.prevChasingPosY) * (double) partialTicks
					- (entitylivingbaseIn.prevPosY + (entitylivingbaseIn.posY - entitylivingbaseIn.prevPosY) * (double) partialTicks);
			double d2 = entitylivingbaseIn.prevChasingPosZ + (entitylivingbaseIn.chasingPosZ - entitylivingbaseIn.prevChasingPosZ) * (double) partialTicks
