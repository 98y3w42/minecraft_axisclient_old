	...
	protected void renderModel(T entitylivingbaseIn, float p_77036_2_, float p_77036_3_, float p_77036_4_, float p_77036_5_, float p_77036_6_, float p_77036_7_) {
		...
		if (flag || flag1) {
			...
			if ((Axis.getAxis().getModuleManager().getModuleByName("WireframeESP").isEnabled()) && ((entitylivingbaseIn instanceof EntityPlayer))) {
				...
				GlStateManager.setColorBuffer(HUD.wirecolor);
				float red = (float) (HUD.wirecolor >> 16 & 255) / 255.0F;
				float blue = (float) (HUD.wirecolor >> 8 & 255) / 255.0F;
				float green = (float) (HUD.wirecolor & 255) / 255.0F;
				float alpha = (float) (HUD.wirecolor >> 24 & 255) / 255.0F;
				GL11.glColor4f(red, blue, green, 1.0f);
				int x = 0;
				ColorUtil.rainbow(index + x * 200000000L, 1.0F).getRGB();
