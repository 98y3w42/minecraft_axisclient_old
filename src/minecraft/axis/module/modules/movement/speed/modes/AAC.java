...
public class AAC extends SpeedMode implements Mode {
	...
	public void onUpdate(UpdateEvent event) {
		if (event.state == Event.State.PRE) {
			...
			if ((mc.gameSettings.keyBindForward.pressed || mc.gameSettings.keyBindLeft.pressed || mc.gameSettings.keyBindRight.pressed || mc.gameSettings.keyBindBack.pressed) && (!this.motionMultiplied)) {
				mc.timer.timerSpeed = 2.5F;
				mc.thePlayer.cameraPitch = 0.1F;
				mc.thePlayer.distanceWalkedModified = 44.0F;
				this.motionMultiplied = true;
			}
			if (mc.gameSettings.keyBindForward.pressed) {
				if (mc.thePlayer.onGround) {
					mc.thePlayer.jump();
					this.motionMultiplied = false;
				} else {
